...
#If False Then
Private eStandard, eFlat, eVistaAero, eVistaToolbar, eInstallShield, eFlatHover
...
Private m_PicSize           As Long                 'Picsize
...
Private m_WindowsNT          As Long                 'Supports Unicode?
...
Dim mIcon As ICONINFO
    ...
    Else
        ...
        If UseMaskColor Then
            m_lMaskColor = GetPixel(lHDC, lLeft, lTop)
            hDCSrc = CreateCompatibleDC(0)
            hOldob = SelectObject(hDCSrc, oPic.Handle)
            TransparentBlt hDC, lLeft, lTop, lWidth, lHeight, hDCSrc, 0, 0, lWidth, lHeight, m_lMaskColor
...
'***************************************************************************
    ...
    Select Case m_ButtonStyle
    Case eWindowsXP, eVistaAero, eVistaToolbar, eInstallShield, eXPToolbar
...
Dim PicSize As Long                       'Picture Size
...
Dim CaptionY As Long                      'Top of Caption
    ...
    PicSize = ScaleY(tmpPic.Height, vbHimetric, vbPixels)
    
    'If you load a picture of 200*100 then it will be cropped to 48*48!!
    'I still dont understand why u need 200*100 size picture in a button<"">
    
    If PicSize > 48 Then PicSize = 48
    
    Select Case m_PictureAlign
    Case epLeftOfCaption
        PicX = (lw - (PicSize + CaptionW)) \ 2
        If PicX < 4 Then PicX = 4
        PicY = (lh - PicSize) \ 2
        CaptionX = (lw \ 2 - CaptionW \ 2) + (PicSize \ 2) + 3 'Some distance of 3
        If CaptionX < (PicSize + 8) Then CaptionX = PicSize + 8  'Text shouldn't draw over picture
        CaptionY = (lh \ 2 - CaptionH \ 2)

    Case epLeftEdge
        PicX = 4
        PicY = (lh - PicSize) \ 2
        CaptionX = (lw \ 2) - (CaptionW \ 2) + (PicSize \ 2)
        If CaptionX < (PicSize + 8) Then CaptionX = PicSize + 8  'Text shouldn't draw over picture
        CaptionY = (lh \ 2 - CaptionH \ 2)
    ...
    Case epRightEdge
        ...
        PicX = lw - PicSize - 4
        PicY = (lh - PicSize) \ 2
        CaptionX = (lw - CaptionW - 4) - PicSize
        CaptionY = (lh \ 2 - CaptionH \ 2)

    Case epRightOfCaption

        PicX = (lw - (PicSize - CaptionW)) \ 2
        If PicX > (lw - PicSize - 4) Then PicX = lw - PicSize - 4
        PicY = (lh - PicSize) \ 2
        CaptionX = (lw \ 2 - CaptionW \ 2) - (PicSize \ 2) - 3
    ...
    Case epBottomEdge
        ...
        PicY = (lh - PicSize) - 4
        CaptionX = (lw \ 2 - CaptionW \ 2)
        CaptionY = (lh \ 2 - PicSize \ 2 - CaptionH \ 2) - 2

    Case epBottomOfCaption
        PicX = (lw - PicSize) \ 2
        PicY = (lh - (PicSize - CaptionH)) \ 2
        If PicY > lh - PicSize - 4 Then PicY = lh - PicSize - 4
        CaptionX = (lw \ 2 - CaptionW \ 2)
        CaptionY = (lh \ 2 - PicSize \ 2 - CaptionH \ 2) - 2
    ...
    If m_Picture Is Nothing Then
        ' --Calculate if no picture available
    ...
    Else
        ...
        Case ecRightAlign
            ...
            ElseIf m_PictureAlign = epLeftEdge Then
                CaptionX = (lw - CaptionW - 4)
                If CaptionX < PicSize + 4 Then
                    CaptionX = PicSize + 4
                End If
            ElseIf m_PictureAlign = epLeftOfCaption Then
                CaptionX = (lw - CaptionW - 4)
                PicX = CaptionX - PicSize - 4
            Else
                CaptionX = (lw - CaptionW - 4)
            End If
        Case ecCenterAlign
    ...
    Case vbPicTypeIcon
        ...
        If m_bEnabled Then
            DrawIconEx UserControl.hDC, PicX, PicY, tmpPic.Handle, m_PicSize, m_PicSize, 0, 0, DI_NORMAL
        Else
            ' --Draw grayed picture (Thanks to Jim Jose)
            PaintGrayScale hDC, tmpPic.Handle, PicX, PicY, PicSize, PicSize
        End If
    ...
    Case vbPicTypeBitmap
        If m_bEnabled Then
            If m_bUseMaskColor Then
                ...
                TransparentBlt hDC, PicX, PicY, PicSize, PicSize, hDCSrc, 0, 0, PicSize, PicSize, m_lMaskColor
                SelectObject hDCSrc, hOldob
                DeleteDC hDCSrc
            Else
                PaintPicture tmpPic, PicX, PicY, PicSize, PicSize
            End If
        Else
            PaintGrayScale hDC, tmpPic.Handle, PicX, PicY, PicSize, PicSize
        End If
...
Dim lpic As New StdPicture
    ...
    hDCSrc = CreateCompatibleDC(0)             'Only for GIFs,JPGs BMPs
    hOldob = SelectObject(hDCSrc, lpic.Handle) 'Icons will be automatically
                                               'made transparent by
    m_lMaskColor = GetPixel(hDCSrc, 0, 0)      'DrawIconEx function
    SelectObject hDCSrc, hOldob
    DeleteDC hDCSrc
